...
void add_file_tracker(){
    ...
    pkt.header.payload_length = strlen(filename) + 1;
    pkt.header.m_addr = self_addr.sin_addr.s_addr;
    pkt.header.m_port = htons(self_addr.sin_port);
    memcpy(pkt.payload, filename, pkt.header.payload_length);


    // send packet to tracker
    int status = sendto(sock, &pkt, sizeof(pkt.header) + pkt.header.payload_length, 0, (struct sockaddr *)&tracker_addr, sizeof(struct sockaddr_in));
    if (status == -1)
    {
        pthread_mutex_lock(&stdout_lock);
...
void receive_available_rooms(packet *pkt)
{
    ...
    */
    printf("Os seguintes usuários possuem o arquivo: \n%s", pkt->payload);
    pthread_mutex_unlock(&stdout_lock);
}
